...
def section_label(section_id):
    if section_id == ENDGAME_SECTION_FORMULA:
        return t("formula")
    if section_id == ENDGAME_SECTION_CHALLENGE:
        return t("challenge")
...
def load_language_pref():
    ...
    candidates = [
        get_user_data_path(LANGUAGE_FILE_NAME),
        find_data_file(LANGUAGE_FILE_NAME),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), LANGUAGE_FILE_NAME),
    ]
    for p in candidates:
...
def save_language_pref(lang):
    ...
    path = get_user_data_path(LANGUAGE_FILE_NAME)
    try:
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
...
class PikafishEngine:
    ...
    def analyse_full(self, fen, movetime_ms=AI_EVAL_MOVETIME_MS):
        """分析局面：分數 + 最佳著 + 主變例 PV。"""
        self._drain_queue()
        self._send(f"position fen {fen}")
    ...
    def _loop(self):
        while not self.stop_event.is_set():
            ...
            try:
                ...
                elif kind == "analyse_full":
                    payload = self.engine.analyse_full(fen, movetime_ms)
                else:
